...
async def _(event):
    ...
    if input_str:
        ...
        if event.is_private:
            return False
        await tr(event, "Searching Participant Lists.")
        p = 0
        async for i in borg.iter_participants(
            ...
            try:
                ...
                await tr(event, str(ex))
            else:
                p += 1
        await tr(event, "{}: {} unbanned".format(event.chat_id, p))
...
async def _(event):
    ...
    if input_str:
        ...
        if not (chat.admin_rights or chat.creator):
            await tr(event, "`You aren't an admin here!`")
            return False
    p = 0
    b = 0
    c = 0
    ...
    async for i in borg.iter_participants(event.chat_id):
        ...
        if isinstance(i.status, UserStatusEmpty):
            ...
            if "y" in input_str:
                ...
                if not status:
                    await tr(event, "I need admin priveleges to perform this action!")
                    e.append(str(e))
                    break
                else:
                    c = c + 1
...
Bots: {}
None: {}"""
        await tr(event, required_string.format(c, p, d, y, m, w, o, q, r, b, n))
